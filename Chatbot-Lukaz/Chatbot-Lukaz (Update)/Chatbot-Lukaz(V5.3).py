...
def update_pairs():
    ...
    pairs = [
       ...
       (r'Qual é a sua versão?|Em qual versão você está?', [
           'Esta versão que você está utilizando é a 5.3'
           ]),
...
def contar_piada():
    ...
    piadas = [
        ...
        "O que um espelho falou para o outro? \
              Não aguento mais refletir sobre a vida!",

        "O que aconteceu com o ladrão que roubou um calendário? \
        Ele pegou 12 meses!",

        "Qual é a maior aranha do mundo? O Macaco aranha!",

        "Por que a girafa tem um pescoço longo? Porque eles não são pequenos!",

        "Qual é o animal mais inteligente? O macaco, \
        porque ele vive tranquilo e não paga impostos!",

        "Por que a água não briga? Porque ela não é esquentada!",

        "Qual é a coisa mais pesada do mundo? A balança!",

        "Por que a Impressora não ri? Porque ela não tem (ton)er de humor!",
...
""").lower()
    ...
    if jog1 not in escolhas:
        if jog1.lower() == "qual função você está executando?":
            return "Estou na função Jokenpo"
        elif jog1.lower() == "em qual função você está?":
            return "Estou na função Jokenpo"

        elif jog1.lower() == "não quero mais jogar jokenpo":
            return "Ok, vou voltar a função de diálogo"
        elif jog1.lower() == "sair jokenpo":
            return "Ok, vou voltar a função de diálogo"
